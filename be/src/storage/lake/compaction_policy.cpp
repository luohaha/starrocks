...
public:
    ...
    StatusOr<std::vector<RowsetPtr>> pick_rowsets(int64_t version) override;
    StatusOr<std::vector<RowsetPtr>> pick_rowsets(TabletMetadataPtr tablet_metadata, bool calc_score, std::vector<bool>* has_dels);
...
StatusOr<uint32_t> primary_compaction_score_by_policy(TabletManager* tablet_mgr, const TabletMetadataPB& metadata) {
    ...
    std::vector<bool> has_dels;
    ASSIGN_OR_RETURN(auto pick_rowsets, policy->pick_rowsets(std::make_shared<TabletMetadataPB>(metadata), true, &has_dels));
