...
Status PersistentIndexSstable::load_delvec(TabletManager* tablet_mgr, int64_t tablet_id) {
    if (!_sstable_pb.need_delvec()) {
        // Only sstables generated by compaction need to load delvec.
        return Status::OK();
    }
    // load delvec
    SegmentReadOptions seg_options;
    auto delvec_loader =
            std::make_unique<LakeDelvecLoader>(tablet_mgr, nullptr, true /* fill cache */, seg_options.lake_io_opts);
    return delvec_loader->load(TabletSegmentId(tablet_id, _sstable_pb.shared_rssid()), _sstable_pb.shared_version(),
                               &_delvec);
}
