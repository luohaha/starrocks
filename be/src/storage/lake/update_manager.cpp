...
Status UpdateManager::_handle_merge_condition(const RowsetUpdateStateParams& params, uint32_t rowset_id,
    ...
    TRACE_COUNTER_INCREMENT("handle_merge_condition_count", 1);
...
Status UpdateManager::_do_update_with_condition_parallel(const RowsetUpdateStateParams& params, uint32_t rowset_id,
    ...
    for (; !upsert->done(); upsert->next()) {
        ...
        auto condition_merge_func = [&, context_ptr, current]() {
            auto st = _handle_merge_condition(params, rowset_id, upsert_idx, upsert.get(), context_ptr, current,
                                              tablet_column, read_column_ids, index);
            if (!st.ok()) {
