...
public:
    ...
    explicit ComparableChunk(Chunk* chunk, size_t order, size_t key_columns, std::vector<uint32_t> sort_key_idxes,
            ...
            : ComparableChunk(chunk, order, key_columns, sort_key_idxes, merge_condition) {
...
protected:
    Status do_get_next(Chunk* chunk, std::vector<RowSourceMask>* source_masks, std::vector<uint64_t>* rssid_rowids);
    ...
    Status do_get_next(Chunk* chunk, std::vector<uint64_t>* rssid_rowids) {
