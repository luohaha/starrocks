...
Status TabletUpdates::recover() {
    ...
    EditVersion latest_applied_version = _edit_version_infos[_apply_version_idx]->version;
    LocalPrimaryKeyRecover recover(&_tablet, StorageEngine::instance()->update_manager(), latest_applied_version);
    RETURN_IF_ERROR(recover.recover());

    // rebuild rewset stats
    for (auto& [rsid, rowset] : _rowsets) {
        auto stats = std::make_unique<RowsetStats>();
        stats->num_segments = rowset->num_segments();
        stats->num_rows = rowset->num_rows();
