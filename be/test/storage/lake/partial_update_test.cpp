...
TEST_P(LakePartialUpdateTest, test_partial_update_retry_rewrite_check) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (expect_segment_size == -1) {
            expect_segment_size = FileSystem::Default()->get_file_size(filename).value();
        } else {
            // make sure each rewrite will generate same file size
            EXPECT_TRUE(expect_segment_size == FileSystem::Default()->get_file_size(filename).value());
        }
    }
}
